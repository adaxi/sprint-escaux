...
const Syntax = Me.imports.xml.syntax.Syntax;
...
const parse = (function() {
  ...
  return parseXml = function (xml, options = emptyObject) {
  ...
  function consumeCharData(state) {
    let result = scan(state, Syntax.Anchored.CharData);
    let text = result[0];
  ...
  function consumeComment(state) {
    let result = scan(state, Syntax.Anchored.Comment);
  ...
  function consumeElement(state) {
    let result = scan(state, Syntax.Anchored.EmptyElemTag);
    let tag = result[0];
    let name = result[1];
    let attrs = result[2];
    ...
    if (!isEmpty) {
      result = scan(state, Syntax.Anchored.STag);
      tag = result[0];
      name = result[1];
      attrs = result[2];
    ...
    if (!isEmpty) {
      ...
      let result = scan(state, Syntax.Anchored.ETag);
      let endName = result[1];
  ...
  function consumePI(state) {
    let result = scan(state, Syntax.Anchored.PI);
    let match = result[0];
    let target = result[1];
  ...
  function consumeReference(state) {
    let result = scan(state, Syntax.Anchored.Reference);
    let ref = result[0];
